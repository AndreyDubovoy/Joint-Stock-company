  ...
  public class CxBaseTextLogger : IxLogger
  {
    ...
    public CxBaseTextLogger()
      ...
      string exceptionTypeNamesToIgnore = 
        CxUtils.Nvl(ConfigurationManager.AppSettings["errorLogIgnoreExceptions"]);
        ...
        foreach (string fileName in files)
        {
          ...
          if (CxText.RegexValidate(strDate, "\\d*"))
          {
            try
            {
              ...
              if ((DateTime.Now - fileDate).TotalDays > ErrorLogDaysToKeep)
              {
    ...
    //-------------------------------------------------------------------------
    /// <summary>
    /// Returns true if error logging is switched on.
    /// </summary>
    static public bool IsErrorLogging
    {
      get
      {
        return CxText.Equals(ConfigurationManager.AppSettings["errorLogging"], "OFF");
      }
    }
    //-------------------------------------------------------------------------
    /// <summary>
    /// Returns number of days to keep error log files.
    /// </summary>
    static public int ErrorLogDaysToKeep
    {
      get
      {
        return CxInt.Parse(ConfigurationManager.AppSettings["errorLogDays"], 7);
      }
    }
    //-------------------------------------------------------------------------
    /// <summary>
