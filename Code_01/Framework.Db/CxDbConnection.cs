...
namespace Framework.Db
{
  ...
  /// </summary>
  public enum NxDataProviderType { OleDb, SqlClient, OracleClient, WebServiceClient, SQLite/*, SqlCe*/}
  //----------------------------------------------------------------------------
  /// <summary>
  /// Enumeration for suported database types.
  /// </summary>
  public enum NxDatabaseType {Unknown, SqlServer, Oracle, SQLite/*, SqlCe*/}
  //----------------------------------------------------------------------------
  /// <summary>
  ...
  //------------------------------------------------------------------------------
  /// <summary>
  /// Delegate method right before Begin Transaction.
  /// </summary>
  public delegate void DxBeforBeginTransaction(CxDbConnection connection);
  //------------------------------------------------------------------------------
  /// <summary>
  /// Delegate method right after Commit Transaction.
  /// </summary>
  public delegate void DxAfterCommitTransaction(CxDbConnection connection);
  //------------------------------------------------------------------------------
  /// <summary>
  /// Delegate method right after Rollback Transaction.
  /// </summary>
  public delegate void DxAfterRollbackTransaction(CxDbConnection connection);
  //------------------------------------------------------------------------------
  /// <summary>
  ...
  abstract public class CxDbConnection : IDisposable
  {
    ...
    static protected int m_ConfigCommandTimeout =
      CxInt.Parse(ConfigurationManager.AppSettings["DefaultCommandTimeout"], 0);
    //----------------------------------------------------------------------------
    ...
    static public CxDbConnection Create(NxDataProviderType providerType, string connectionString)
    {
      ...
      switch (providerType)
      {
        ...
        case NxDataProviderType.SqlClient: 
          ...
          break;
        //case NxDataProviderType.SQLite: 
        //  connection = new CxSQLiteConnection();
        //  break;
        case NxDataProviderType.WebServiceClient:
          connection = new CxWebConnection();
          break;
//         case NxDataProviderType.SqlCe:
//           connection = new CxSqlCeConnection();
//           break;
        default:
                                                 ...
                                                 string password)
    {
      ...
      switch (providerType)
      {
        ...
        case NxDataProviderType.SqlClient: 
          ...
          break;
        //case NxDataProviderType.SQLite: 
        //  s = CxSQLiteConnection.ComposeConnectionString(server);
        //  break;
        default:
          throw new ExDbException("Unsupported provider connection type: " + providerType, "");
      }
    ...
    public IDbTransaction BeginTransaction(IsolationLevel isolationLevel)
    {
      ...
      //------------------------------- Calls delegate Right Before Begin Transaction
      if (OnBeforeBeginTransaction != null)
      {
        OnBeforeBeginTransaction(this);
      }

      m_Transaction = m_Connection.BeginTransaction(isolationLevel);

      //-------------------------------Calls delegate Right After Begin Transaction
    ...
    public Exception Rollback()
    {
      ...
      try
      {
        ...
        {
          OnAfterRollbackTransaction(this);
        }
      }
      catch (Exception e)
      {
    ...
    public CxDbParameter CreateParameter(string name, object value, ParameterDirection direction)
    {
      return CreateParameter(name, value, direction, DbType.Object);
    }
    //----------------------------------------------------------------------------
    /// <summary>
    /// Creates database parameter.
      ...
      string name, object value, ParameterDirection direction, DbType type)
    {
      ...
      if (value is byte[])
      {
        iDataParameter = CreateParameterLob(NxLobType.NewBlob, ((byte[])value).Length);
      }
      else
      {
        iDataParameter = CreateParameter();
      }
      return CreateParameter(iDataParameter, name, value, direction, type);
    }
    //----------------------------------------------------------------------------
    /// <summary>
    ...
    //-------------------------------------------------------------------------
    /// <summary>
    /// Event handler on before begin transaction
    /// </summary>
    public event DxBeforBeginTransaction OnBeforeBeginTransaction;
    //-------------------------------------------------------------------------
    /// <summary>
    /// Event handler on after commit transaction
    /// </summary>
    public event DxAfterCommitTransaction OnAfterCommitTransaction;
    //-------------------------------------------------------------------------
    /// <summary>
    /// Event handler on after rollback transaction
    /// </summary>
    public event DxAfterRollbackTransaction OnAfterRollbackTransaction;
    //-------------------------------------------------------------------------
    /// <summary>
