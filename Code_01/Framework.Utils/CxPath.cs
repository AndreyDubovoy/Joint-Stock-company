...
namespace Framework.Utils
{
  ...
  public class CxPath
  {
    ...
    /// <returns>array of file names</returns>
    static public bool CopyFileAndOwerwrite(string source, string target)
    {
      try
      {
        var curAttr = FileAttributes.Normal;
        if (File.Exists(target))
        {
          curAttr = File.GetAttributes(target);
          File.SetAttributes(target, FileAttributes.Normal);
        }
        File.Copy(source, target, true);
        if (curAttr != FileAttributes.Normal)
          File.SetAttributes(target, curAttr);
        return true;
      }
      catch
      {
    ...
    /// <summary>
    /// Combines path. Returns combined path even if path2, path3 ... start with '\\'. 
    /// Path.Combine returns path2 if it starts with '\\'.
    /// </summary>
    ...
    {
      return Path.Combine(path1, path2.TrimStart('\\'), path3.TrimStart('\\'), path4.TrimStart('\\'),
        path5.TrimStart('\\'));
    }
    //--------------------------------------------------------------------------
  }
